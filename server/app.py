...
def create_phrase():
    ...
    phrase: models.Phrase = session.query(models.Phrase).filter(
        models.Phrase.phrase == phrase_name
    ).one_or_none()
...
def vote_on_phrase():
    ...
    if not (phrase_id and humor and severity and importance):
        return Response(
            response={
                "error": "phrase_id, humor, severity, and importance are required fields"
            },
            status=400,
        )
    ...
    vote: models.Vote = models.Vote(
        phrase_id=phrase_id, humor=humor, severity=severity, importance=importance
    )
    session.add(vote)
    session.commit()
